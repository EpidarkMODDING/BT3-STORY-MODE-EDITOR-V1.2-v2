...
class BT3CombinedEditor:
  ...
  def __init__(self, root):
    ...
    menubar = tk.Menu(
        root, bg="#2d2d2d", fg="#ffffff", activebackground="#444444"
    )
    file_menu = tk.Menu(
        menubar, tearoff=0, bg="#2d2d2d", fg="#ffffff", activebackground="#444444"
    )
    ...
    self.lbl_file.pack(anchor="ne", padx=10, pady=2)
    ...
    self.main_notebook.add(
        self.tab_anim_editor, text=" Animaciones ($anim) "
    )
    self.main_notebook.add(
        self.tab_zit_editor, text=" Z-Items ($zit) "
    )
  ...
  def setup_dark_theme(self):
    ...
    style.configure(
        "TLabelframe.Label", background=frame_bg, foreground=fg_color
    )
    style.configure("TFrame", background=bg_color)
    style.configure(
        "TButton", background="#3c3c3c", foreground=fg_color, borderwidth=1
    )
    style.map("TButton", background=[("active", accent_color)])
    style.configure(
        "TCombobox", fieldbackground="#3c3c3c", background="#3c3c3c", foreground=fg_color
    )
    style.map(
        "TCombobox",
    ...
    style.configure(
        "TEntry", fieldbackground="#3c3c3c", foreground=fg_color, insertcolor=fg_color
    )
    style.configure("TNotebook", background=bg_color, borderwidth=0)
    style.configure(
        "TNotebook.Tab", background="#2d2d2d", foreground=fg_color, padding=[10, 4]
    )
    style.map(
        "TNotebook.Tab",
    ...
    style.configure(
        "Treeview.Heading", background="#333333", foreground=fg_color, borderwidth=1
    )
